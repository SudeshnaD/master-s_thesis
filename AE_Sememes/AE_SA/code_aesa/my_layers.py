...
class SememeAttention(Layer):
    ...
    def __init__(self,
        ...
        self.bias = bias
        super(SememeAttention, self).__init__(**kwargs)
    ...
    def call(self, input_tensor, mask=None):
        ...
        # d=[]
        # for i in range(K.int_shape(x)[1]):
        #     #d.append(K.tanh(K.dot(x[i],y[i])))
        #     d.append(K.dot(x[i],K.reshape(y[0][i],(100,1))))
        d = K.tanh(d)
        exp = K.exp(d)
        sm=exp / K.sum(exp, axis=-2, keepdims=True)
        return sm
...
class WeightedSum(Layer):
    ...
    def call(self, input_tensor, mask=None):
        ...
        x_ti = K.sum(a*x, axis=1)
        return x_ti
        #return K.expand_dims(x_ti, axis=1)
...
class MaxMargin(Layer):
    ...
    def call(self, input_tensor, mask=None):
        ...
        loss = K.sum(K.maximum(0., (1. - pos + neg)), axis=-1, keepdims=True)
        return loss
