...
def create_vocab(domain, maxlen=0, vocab_size=0):
    ...
    #source='test/Test_unlabelledTrain.txt'
    #source = "test/train_dev_labelled_unlabelled.txt"
    ...
    vocab_file = codecs.open('../preprocessed_data/sentihood/vocab', mode='w', encoding='utf8')
...
def read_dataset(domain, phase, vocab, maxlen):
    # assert domain in {'restaurant', 'beer'}
    assert phase in {'train', 'test','test_filtered'} #added test_filtered to use test_filtered.txt for evaluation : SD
...
def get_data(domain, vocab_size=0, maxlen=0):
    ...
    #test_x, test_maxlen = read_dataset(domain, 'test_filtered', vocab, maxlen) #changed to access test_filtered.txt : SD
