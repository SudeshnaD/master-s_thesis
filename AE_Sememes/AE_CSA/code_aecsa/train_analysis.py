...
model = create_model(args, overall_maxlen, vocab, emb_reader)
...
from Neg_emlayer import Neg_embedding
...
for ii in range(args.epochs):
    ...
    #for b in range(0,1):
        ...
        print(np.array(sen_input.shape))
        neg_input = next(neg_gen)

        sem = Sememe(vocab, sen_input, emb_reader)
        input_sememe, input_average, input_embedding = sem.run_script()
        print(np.array(input_sememe).shape, np.array(input_average).shape)
        ng = Neg_embedding(vocab, neg_input, emb_reader)
        neg_avg_embed = ng.run_script()
        print(np.array(neg_avg_embed).shape)


        batch_loss, batch_max_margin_loss = model.train_on_batch([input_embedding, input_sememe, input_average, neg_avg_embed],
    ...
    if loss < min_loss:
        ...
        model.save(out_dir + '/model_param{}'.format(args.aspect_size))
