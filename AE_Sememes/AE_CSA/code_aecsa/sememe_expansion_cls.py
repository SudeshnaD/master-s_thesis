...
class Sememe:
    ...
    def sentence_embedding(self, sen_idx, senth_emb_v):
        sen_emb=[]
        for word in sen_idx: #list of tokens
            sen_emb.append(senth_emb_v[word])
        return np.array(sen_emb)
    ...
    def run_script(self):
        ...
        for sen_idx in self.sen_idx_b:
            ...
            sen_lemma_b.append(sen_lemma)
            batch_average.append(S)
            sen_b_embedding.append(self.sentence_embedding(sen_idx, senth_emb_v))
        return sen_lemma_b, batch_average, sen_b_embedding
