...
parser.add_argument("-aspect_size", "--aspect_size", dest="aspect_size", type=int, metavar='<int>', default=14,
...
print(overall_maxlen, len(train_x[0]), len(test_x[0]))
...
for ii in range(args.epochs):
    ...
    if loss < min_loss:
        ...
        for ind in range(len(aspect_emb)):
            ...
            print([sims[w] for w in ordered_words[:100]][:10])
            global_sim.append([sims[w] for w in ordered_words[:100]])
    

    if len(global_sim)==args.aspect_size:
        cos_sim_g=global_sim
        with open('FTcos_sim_g{}.txt'.format(str(args.aspect_size)),'w') as f:
           f.write(str(global_sim))
...
""" from Elbow_args import elbow
if len(cos_sim_g)==aspect_size:
    stats=elbow(cos_sim_g)
    print(stats)
    with open('avg_meanavg.txt','a') as f:
                f.write(str(stats)) """
