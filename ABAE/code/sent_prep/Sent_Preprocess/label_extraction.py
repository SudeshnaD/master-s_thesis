...
def load_task(data_dir):
    ...
    #extract_label(in_file)  #commented as test_labels are already extracted in path test data_labels/test_set.txt
...
def extract_label(in_file,phase):
    with open(in_file) as f:
        data = json.load(f)
    test_set=[]
    test_labels=[]
    out_dir='test data_labels'
    for d in data:
        text = d['text']
        opinions = []
        aspect=''
    ...
    print('No. of data in the file: ',len(test_set),len(test_labels))
    #out=open(out_dir + '/'+phase+'.txt', 'w', encoding='utf-8')
    #out=open("test data_labels/test_set.txt", "w", encoding='utf-8')
    #out.write(str(test_set))
    labelfile=out_dir + '/'+phase+'_labels.txt'
    with open(labelfile, "w", encoding='utf-8') as out:
